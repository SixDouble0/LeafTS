// Mock MMIO test for STM32L5 flash HAL.
//
// Compiled with -DLEAFTS_MOCK_FLASH.  Runs on host (x86/x64).
//
// Register layout same as STM32L4 (KEYR@+0x08, SR@+0x10, CR@+0x14).
// L5 has an 8-bit page-number field (PNB [10:3]) and a BKER bank-select bit.
...
void setUp(void) {
    ...
    _mock_set_flash_bases((uintptr_t)mock_regs,
                          ...
                          TEST_LOG_BASE);
    stm32l5_flash_init(&hal, TEST_LOG_BASE, TEST_SIZE);
}
...
void test_erase_clears_per_bker_strt_after_completion(void) {
    hal.erase(TEST_LOG_BASE);
    TEST_ASSERT_EQUAL_INT(0, MOCK_CR & (CR_PER | CR_BKER | CR_STRT));
}
...
int main(void) {
    ...
    RUN_TEST(test_erase_clears_per_bker_strt_after_completion);
