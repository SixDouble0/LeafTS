// Mock MMIO test for STM32F2 flash HAL.
//
// Compiled with -DLEAFTS_MOCK_FLASH.  Runs on host (x86/x64).
//
// F4-style register layout: KEYR@+0x04, SR@+0x0C, CR@+0x10.
// Erase is sector-based via addr_to_sector().  TEST_LOG_BASE must align
// to a sector boundary listed in the F2 sector table (sector 5 = 0x08020000).
#include "../unity/unity.h"
#include "../hal/stm32/hal_stm32f2_flash.h"
#include <string.h>
#include <stdint.h>

// 5 entries covers CR at offset +0x10 (index 4).
...
// Sector 5 of STM32F2 starts at 0x08020000 and is 128 KB.
...
void setUp(void) {
    ...
    _mock_set_flash_bases((uintptr_t)mock_regs,
                          ...
                          TEST_LOG_BASE);
    stm32f2_flash_init(&hal, TEST_LOG_BASE, TEST_SIZE);
}
