// Mock MMIO test for STM32G4 flash HAL.
//
// Compiled with -DLEAFTS_MOCK_FLASH.  Runs on host (x86/x64).
//
// Register layout identical to STM32L4:
...
void setUp(void) {
    ...
    _mock_set_flash_bases((uintptr_t)mock_regs,
                          ...
                          TEST_LOG_BASE);
    stm32g4_flash_init(&hal, TEST_LOG_BASE, TEST_SIZE);
}
...
void test_read_returns_ff_on_fresh_erased_flash(void) {
    ...
    for (int i = 0; i < 16; i++) TEST_ASSERT_EQUAL_HEX8(0xFF, buf[i]);
}

void test_read_after_write_returns_correct_data(void) {
    uint32_t addr = TEST_LOG_BASE + PAGE * 3;
    uint8_t  data[DWORD] = {0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF, 0x11, 0x22};
    uint8_t  buf [DWORD] = {0};
...
int main(void) {
    ...
    RUN_TEST(test_read_after_write_returns_correct_data);
    return UNITY_END();
}
