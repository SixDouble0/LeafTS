...
// 256 KB is too large for a real stack array; use the minimum that exercises
// the sector: the sector table address must match for addr_to_sector().
// We only allocate a small window and redirect the logical page inside it.
#define TEST_LOG_BASE  0x08040000UL
...
void setUp(void) {
    ...
    _mock_set_flash_bases((uintptr_t)mock_regs,
                          ...
                          TEST_LOG_BASE);
    stm32f7_flash_init(&hal, TEST_LOG_BASE, TEST_SIZE);
}
