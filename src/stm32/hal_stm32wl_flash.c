// STM32WL Flash HAL for LeafTS
// Reference: RM0453 (STM32WL55/WL54/WLE5/WLE4)
//
// Flash controller nearly identical to STM32L4 (same CR bit layout).
// Key differences: FLASH_REG_BASE = 0x58004000 (not 0x40022000),
//                  page size = 2048 bytes, single bank only.
...
#define FLASH_REG_BASE  0x58004000UL   // CPU1 secure peripheral space
#endif
...
static int wl_read(uint32_t address, uint8_t *buffer, size_t size) {
    if (!buffer || address < g_base || address + size > g_base + g_size) return -1;
    memcpy(buffer, flash_ptr(address), size);
    return 0;
}
static int wl_write(uint32_t address, const uint8_t *buffer, size_t size) {
    if (!buffer || address < g_base || address + size > g_base + g_size) return -1;
...
static int wl_erase(uint32_t sector_address) {
...
int stm32wl_flash_init(hal_flash_t *flash, uint32_t flash_base, uint32_t total_size) {
    ...
    flash->read = wl_read; flash->write = wl_write; flash->erase = wl_erase;
